const EventMapHeader = () => {
  return (
    <div className="h-[90px] rounded-bl-4xl rounded-br-4xl shadow-lg flex flex-col justify-center items- w-full bg-white z-[50] text-center">
      <h1 className="h-[20px] font-normal gap-2">
        <strong className="text-[#DD1C1C] font-normal">E</strong>VENT CAL
        <strong className="text-[#DD1C1C] font-normal">E</strong>NDER
      </h1>
      <p className="text-[12px]">イベントカレンダー</p>
    </div>
  );
};

export default EventMapHeader;

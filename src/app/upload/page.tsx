...
export default function Upload() {
  ...
  const handleUpload = async () => {
    if (!file) {
      return setPopup({ open: true, type: "error", message: "Please select a file!" });
    }
    ...
    try {
      ...
      if (response.ok) {
        setPopup({ open: true, type: "success", message: "File uploaded and converted successfully!" });
        setFile(null);
      } else {
        setPopup({ open: true, type: "error", message: "Error uploading file!" });
      }
    } catch (error) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100 p-6">
      <div className="bg-white shadow-lg rounded-xl p-8 w-full max-w-md text-center border border-gray-200">
        <h1 className="text-2xl font-semibold text-[#E00400] mb-6">Upload CSV or Excel File</h1>

        <label
          className={`cursor-pointer flex flex-col items-center p-6 border-2 border-dashed 
            rounded-lg transition duration-300 ${dragging ? "border-[#E00400] bg-red-50" : "border-gray-300 hover:border-[#E00400]"}`}
          onDragOver={(e) => {
            e.preventDefault();
            setDragging(true);
          }}
        ...
        >
          ...
          <span className="text-gray-600 mt-2">Click to upload or drag & drop</span>
      ...
      <Dialog open={popup.open} onOpenChange={(open) => setPopup({ ...popup, open })}>
        <DialogContent className="sm:max-w-md bg-white rounded-xl shadow-2xl p-6 border border-gray-200 transition-transform duration-200 transform scale-95">
          <DialogHeader className="flex flex-col items-center">
            ...
            <DialogTitle className={popup.type === "success" ? "text-green-600" : "text-[#E00400]"}>
              {popup.type === "success" ? "Success" : "Error"}
            </DialogTitle>
            <DialogDescription className="text-gray-700 text-center">{popup.message}</DialogDescription>
          </DialogHeader>
